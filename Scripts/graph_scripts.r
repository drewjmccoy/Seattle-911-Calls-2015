...
day_graph <- function(data) {
  margin <- list(t = 50)
  graph <- data %>% plot_ly(x = date,
                            y = calls,
                            mode = "markers") %>%
    layout(
      title = "Calls by Day of the Year",
...
time_graph <- function(data) {
  margin <- list(t = 50)
  graph <- data %>% plot_ly(x = time,
                            y = calls,
                            mode = "markers") %>%
    layout(
      title = "Calls by Time of the Day",
      yaxis = list(title = "Number of Calls"),
...
call_breakdown_graph <- function(data, slice) {
  ...
  # create graph
  m1 <- list (b = bot_margin,
              t = top_margin,
              r = 125)
  f <- list(size = 10)
  #set x axis title
  x1 <- list (title = x_axis_title,
              tickfont = f)
  #set y axis title
  y1 <- list (title = "Instances",
              tickfont = f,
              range = c(0, 10000))
...
specific_breakdown_graph <- function(data, crime_type) {
  ...
  specific_data <- data %>%
    ...
    arrange(-instances)
  bot_margin <- 100
  if (crime_type == "TRAFFIC RELATED CALLS" |
      crime_type == "BURGLARY") {
    bot_margin <- 200
  }
  ...
  individual_plot <- specific_data %>% plot_ly(
    ...
    layout(
      title = paste(
        "Breakdown of 2015 Seattle 911 Calls for", crime_type, "by Subgroup", sep = " "
      ),
      xaxis = x2,
      yaxis = y2,
      margin = m2
